...
const AllUsers = () => {
  ...
  //     queryKey:['users'],
  //     queryFn: async()=>{
  //         const res = await fetch('https://online-resel-server.vercel.app/users');
  ...
  useEffect(() => {
    fetch('https://online-resel-server.vercel.app/users')
      .then(res => res.json())
  ...
  const handleDelete = (id) => {
    ...
    if (proceed) {
      fetch(`https://online-resel-server.vercel.app/users/${id}`, {
        method: 'DELETE'
      })
        .then(res => res.json())
        .then(data => {
  ...
  const handleMakeAdmin= id =>{
    fetch(`https://online-resel-server.vercel.app/users/admin/${id}`,{
     method:'PUT',
     headers:
     {
